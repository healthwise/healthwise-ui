...
import styles from './IconArray.css'

class IconArray extends Component {
  getCountMarker(keyBase, count = 0) {
    return (
      <span
        className={'hw-icon-break-row-count ' + styles.icon_row_break_count}
  ...
  getNeutralIcon(key, iconStyle = 'icon_off') {
    return (
      <span className={`${styles.icon} ${styles[iconStyle]} hw-array-${iconStyle}`} key={key}>
        <svg
          role="presentation"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 50 80"
        ...
        >
          <g>
            ...
            />
            <circle strokeWidth="3" cy="20.5" cx="25" r="9.0625" />
          </g>
        </svg>
      </span>
    )
  ...
  getRowBreak(keyBase, count) {
    return (
      <div className={styles.icon_row_break} key={`row-break-${keyBase}-${count}`}>
  ...
  getIconRow(keyBase, icons, count) {
    return (
      <div className={styles.icon_row} key={`row-${keyBase}-${count}`}>
        {icons}
      </div>
    )
  }
  ...
  render() {
    ...
    return (
      <div aria-hidden className={styles.icon_array}>
        {rows}
      </div>
    )
  }
